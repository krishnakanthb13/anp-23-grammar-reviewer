...
export async function handleRunReview(app, itemIndex = -1, promptOverride = "") {
  ...
  // Automatically skip blank or whitespace-only items without sending to AI
  ...
  try {
    ...
    // Record successful AI request attempt
    ...
    // Record failed AI request attempt
...
export async function handleReviewAll(app) {
  ...
  for (let i = 0; i < session.items.length; i++) {
    ...
    if (item.isInspectable && (item.status === "pending" || item.status === "error")) {
      // Check if item is blank (empty or only whitespace)
      if (!item.original || item.original.trim().length === 0) {
        item.status = "accepted";
        continue;
      }
